...
if (localStorage.getItem('savedDate') !== null){ 
    ...
    function dateChange(){
        if (
            (tDate.getDate() - sDate.getDate() > 1 || (tDate.getDate() - sDate.getDate() < 0 && tDate.getHours() > 4)) ||
...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        ...
        if (request.greeting === "ready" && (localStorage.getItem("dateSwitcher") === "on")){
            sendResponse({farewell: "clearData"});
            localStorage.setItem("dateSwitcher", "off");
        } else if (request.greeting === "giveData"){
            sendResponse({farewell: forEgr});
        } else if (request.greeting.length === 9){
            forEgr = request.greeting;
        }
    });
